...
void display(int nEntries = -1) {
	...
	TH1D *time_SIPM_PMT1 = new TH1D("time_SIPM_PMT1","time resolution detector to PMT1",100,-1000.,1000.);
	...
	for(int iEntry = 0; iEntry < nEntries; iEntry++){
		...
		//Baseline
			...
			canvas->cd(4);
			myGraph2 = new TGraph(*data_time, *data_wave);
			myGraph3 = new TGraph(*PMT1_time, *PMT1_wave);
			myGraph4 = new TGraph(*PMT2_time, *PMT2_wave);
			//~ stringstream sstitle;
			sstitle << "all Event " << iEvent << ", time " << eventTime << "s;t / ns;U / mV";
			myGraph2->SetTitle(sstitle.str().c_str());
			myGraph2->SetLineColor(1);
			//~ myGraph2->Draw("AL","same");
			myGraph2->GetXaxis()->SetRangeUser(0,1024);
			myGraph2->GetYaxis()->SetRangeUser(-350,850);
			myGraph3->SetLineColor(2);
			//~ myGraph3->Draw("AL","same");
			myGraph3->GetXaxis()->SetRangeUser(0,1024);
