...
void display(int nEntries = -1) {
	// open the input file
	TFile * file = new TFile((string("./../io/")+string("noise_554V")+string(".root")).c_str(),"READ");
	...
	h1->Draw("HIST");
	canvas1->SaveAs((string("max_position_")+string("noise_554V")+string(".pdf")).c_str());
	delete canvas1;
	TCanvas* canvas_finger = new TCanvas("canvas_finger", "canvas_finger", 800, 800);
	canvas_finger->SetLogy();
	gStyle->SetOptStat(0);
	Double_t par[9];
	TF1 *g1    = new TF1("g1","gaus",13.5,17.5);
    TF1 *g2    = new TF1("g2","gaus",22,30);
    TF1 *g3    = new TF1("g3","gaus",35,45);
    ...
    canvas_finger->Update();
 	finger->Draw("SAME");
	pt->Draw("SAME");
	canvas_finger->SaveAs((string("fingerspectrum_")+string("noise_554V")+string(".pdf")).c_str());
	delete canvas_finger;

	TFile f("finger554.root", "RECREATE");
	finger->Write();
	h1->Write();
	f.Close();
