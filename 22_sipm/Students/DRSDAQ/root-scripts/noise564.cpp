...
void display(int nEntries = -1) {
	...
	for(int iEntry = 0; iEntry < nEntries; iEntry++){
		...
		Double_t max = data_wave->GetSub(538, 564).Max(); // range constrain to avoid spikes from max_position_... plot
    ...
    TF1 *g2    = new TF1("g2","gaus",30,38);
    TF1 *g3    = new TF1("g3","gaus",44,52);
	...
	TFile f("finger564.root", "RECREATE");
	finger->Write();
	h1->Write();
	f.Close();
