...
void display(int nEntries = -1) {
	// open the input file
	TFile * file = new TFile((string("./../io/")+string("noise_574V")+string(".root")).c_str(),"READ");
	...
	for(int iEntry = 0; iEntry < nEntries; iEntry++){
		...
		Double_t max = data_wave->GetSub(542, 568).Max(); // range constrain to avoid spikes from max_position_... plot
    ...
    canvas_finger->Update();
 	finger->Draw("SAME");
	pt->Draw("SAME");
	canvas_finger->SaveAs((string("fingerspectrum_")+string("noise_574V")+string(".pdf")).c_str());
	delete canvas_finger;

	TFile f("finger574.root", "RECREATE");
	finger->Write();
	h1->Write();
	f.Close();
