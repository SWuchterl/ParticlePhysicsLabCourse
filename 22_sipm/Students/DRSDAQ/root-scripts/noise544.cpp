...
void display(int nEntries = -1) {
	// open the input file
	TFile * file = new TFile((string("./../io/")+string("noise_544V")+string(".root")).c_str(),"READ");
	...
	for(int iEntry = 0; iEntry < nEntries; iEntry++){
		...
		Double_t max = data_wave->GetSub(535, 565).Max(); // range constrain to avoid spikes from max_position_... plot
	...
	h1->Draw("HIST");
	canvas1->SaveAs((string("max_position_")+string("noise_544V")+string(".pdf")).c_str());
	delete canvas1;
	TCanvas* canvas_finger = new TCanvas("canvas_finger", "canvas_finger", 800, 800);
	canvas_finger->SetLogy();
	gStyle->SetOptStat(0);
	Double_t par[9];
	TF1 *g1    = new TF1("g1","gaus",14.5,20);
    TF1 *g2    = new TF1("g2","gaus",22,40);
    ...
    canvas_finger->Update();
 	finger->Draw("SAME");
	pt->Draw("SAME");
	canvas_finger->SaveAs((string("fingerspectrum_")+string("noise_544V")+string(".pdf")).c_str());
	delete canvas_finger;

	TFile f("finger544.root", "RECREATE");
	finger->Write();
	h1->Write();
	f.Close();
